...
def main() -> None:
    parser = ArgumentParser(description="Run Phase 10 Explainability Engine (LLM Narrative & Tagging).")
    parser.add_argument(
        "--processed-dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of candidates to explain.",
    )
    ...
    explanation_generator = ExplanationGenerator(llm_provider)

    # 3. Load jobs & run explainability engine
    ...
    print("\n================== TalentGraph Explainability Engine ==================")
    for job in jobs:
        ...
        original_rank_map = {item[0].candidate_id: idx for idx, item in enumerate(scored_candidates, start=1)}
        ...
        for idx, item in enumerate(reranked_results[:args.top_n], start=1):
            ...
            # Generate natural language explanation (LLM or template fallback)
            explained = explanation_generator.generate_explanation(
                ...
                features=f_vector,
                tags=tags,
                candidate_skills=resume.raw_skills if resume else None,
            )

            print(f"\n  Rank {idx}. Candidate ID: {cid} | Blended Score: {item['blended_score']:.4f}")
            print(f"    Tags: {', '.join(explained.tags) if explained.tags else 'None'}")
            print(f"    Recruiter Narrative: {explained.narrative}")
            print("    Key Match Highlights:")
            for pt in explained.matched_points:
                print(f"      - {pt}")
            if explained.missing_points:
                print("    Areas for Development:")
                for pt in explained.missing_points:
                    print(f"      - {pt}")
